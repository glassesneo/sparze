...
pub fn FixedWorld(Components: anytype) type {
    ...
    return struct {
        ...
        pub const SystemType = fn (*Self) anyerror!void;
        pub const SystemPointerType = *const SystemType;

        /// SingleQuery provides iteration over entities with a specific component
        ...
        pub fn Group(comptime GroupComponents: type) type {
            return struct {
                ...
                pub fn init(world: *Self) GroupQuery {
                    return .{ .world = world };
                }

                pub fn getEntities(self: GroupQuery) []const Entity {
                    return self.world.getGroupEntities(ComponentTypes) orelse &[_]Entity{};
                }

                pub fn getArrayOf(self: GroupQuery, comptime C: type) []const C {
                    return self.world.getGroupComponents(ComponentTypes, C) orelse &[_]C{};
                }

                pub fn getMutArrayOf(self: GroupQuery, comptime C: type) []C {
                    return self.world.getGroupComponentsMut(ComponentTypes, C) orelse &[_]C{};
                }
            };
        }

        /// Helper to construct system argument types at compile time
        ...
        /// Create a system function that can be called with world.runSystem(system_fn)
        ...
        pub fn runSystem(self: *Self, comptime system_fn: anytype) !void {
            const system = comptime createSystemFunction(system_fn);
            try system(self);
        }
    };
}
...
test "FixedWorld SingleQuery basic iteration" {
    const Position = struct { x: f32, y: f32 };
    const Velocity = struct { dx: f32, dy: f32 };
    ...
    // Query all positions
    const PositionQuery = World.SingleQuery(Position);
...
test "FixedWorld Group query basic usage" {
    const Position = struct { x: f32, y: f32 };
    const Velocity = struct { dx: f32, dy: f32 };
    ...
    // Use Group query
    const MovementGroup = World.Group(struct { Position, Velocity });
...
test "FixedWorld system function with SingleQuery" {
    ...
    const World = FixedWorld(struct { Position });

    const UpdatePositions = struct {
        fn system(query: World.SingleQuery(Position)) !void {
...
test "FixedWorld system function with Group" {
    ...
    const MovementSystem = struct {
        fn system(group: World.Group(struct { Position, Velocity })) !void {
...
test "FixedWorld system with multiple queries" {
    ...
    const ComplexSystem = struct {
        fn system(
            movement: World.Group(struct { Position, Velocity }),
